...
export const CategoryTabs: React.FC<CategoryTabsProps> = ({
    ...
    const renderTab = (category: Category) => {
        ...
        return (
            ...
            >
                {/* Animated Segment Background - Using Brand Deep Green */}
                {isSelected && (
                    <MotiView
                        ...
                        className="absolute inset-0 bg-orange-500 rounded-xl shadow-md"
                    />
                )}
                ...
                <FontAwesome6
                    ...
                    size={14}
                    color={isSelected ? '#FFFFFF' : '#1C3516'}
                />

                <Text
                    numberOfLines={1}
                    className={`ml-2 text-xs font-bold tracking-tight ${
                        isSelected ? 'text-white' : 'text-[#1C3516]'
                    }`}
                >
                    {displayName}
                </Text>
                ...
                {category.products_count != null && category.products_count > 0 && (
                    <View
                        className={`ml-2 px-1.5 py-0.5 rounded-md ${
                            isSelected ? 'bg-white/25' : 'bg-[#1C3516]/10'
                        }`}
                    >
                        <Text
    ...
    return (
        <View className="bg-[#FDFBF7] pb-3 px-3">
            <View className="bg-white border border-[#1C3516]/15 p-1 rounded-2xl shadow-sm">
                {isCompactList ? (
